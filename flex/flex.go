...
type Flex struct {
	*tview.Box
	focusDelegate func(p tview.Primitive)
	items         []*flexItem
	direction     int
	fullScreen    bool
}
...
func (f *Flex) GetItem(index int) tview.Primitive {
...
func (f *Flex) ResizeItem(p tview.Primitive, fixedSize, proportion int) *Flex {
...
func (f *Flex) MouseHandler() func(action tview.MouseAction, event *tcell.EventMouse, setFocus func(p tview.Primitive)) (consumed bool, capture tview.Primitive) {
...
func (f *Flex) InputHandler() func(event *tcell.EventKey, setFocus func(p tview.Primitive)) {
	return f.WrapInputHandler(func(event *tcell.EventKey, setFocus func(p tview.Primitive)) {
		if event.Key() == tcell.KeyCtrlH {
			i := f.GetFocusedItemIndex() + 1
			if i >= f.GetItemCount() {
				i = 0
			}
			f.SetFocusIndex(i)
...
func (f *Flex) PasteHandler() func(pastedText string, setFocus func(p tview.Primitive)) {
...
func (f *Flex) GetFocusedItemIndex() int {
...
func (f *Flex) SetFocusIndex(index int) {
	if index < 0 || index >= f.GetItemCount() {
		return
	}

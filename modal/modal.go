...
// Focus is called when this tview.Primitive receives focus.
func (m *Modal) Focus(delegate func(p tview.Primitive)) {
	delegate(m.form)
}

// HasFocus returns whether or not this tview.Primitive has focus.
func (m *Modal) HasFocus() bool {
	return m.form.HasFocus()
}

// Draw draws this tview.Primitive onto the screen.
func (m *Modal) Draw(screen tcell.Screen) {
	// Calculate the width of this modal.
	buttonsWidth := 0
	for _, buttonLabel := range m.buttonLabels {
		buttonsWidth += tview.TaggedStringWidth(buttonLabel) + 4 + 2
	}
	buttonsWidth -= 2
	screenX, screenY, screenWidth, screenHeight := m.GetInnerRect()
	width := screenWidth / 3
	...
	m.SetRect(screenX+x, screenY+y, width, height)
